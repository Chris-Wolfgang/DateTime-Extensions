...
2. Run [`verify_stepinto.py`](verify_stepinto.py) — it drives
   ...
   break in the consumer, step into `DateTimeExtensions.TruncateMilliseconds`,
   and assert the resulting frame is the library's SourceLink-mapped source file
   with symbols loaded. If SourceLink or the sequence points were broken the step
   would land with no source and the script exits non-zero.
...
- `consumer/StepIntoConsumer.csproj` / `Program.cs` — the fixture consumer. It
  references `Wolfgang.Extensions.DateTime` and the break line is
  marked `STEP_INTO_TARGET`.
